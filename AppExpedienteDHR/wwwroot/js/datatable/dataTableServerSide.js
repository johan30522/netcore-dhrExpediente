...
function loadDataTable(tableId, ajaxUrl, columns) {
    ...
    const dataTableConfig = {
        "processing": true,
        "serverSide": true, // Habilitar el procesamiento en el lado del servidor
        "ajax": {
            "url": ajaxUrl,
            "type": "POST"
        },
        "columns": columns,
        "stateSave": true,
        "stateDuration": -1, // Mantener el estado indefinidamente
        "language": {
            "decimal": "",
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        $(`#${tableId}`).DataTable().ajax.reload(); // Recargar la tabla
                    } else {
